...
- 设置按钮已经接入 SettingPanel；开始按钮已经接入摄像机动画，动画结束后暂时输出选角面板日志。关于面板和真正的选角面板仍待后续制作。
...
- 关于按钮和开始按钮动画结束后的选角面板显示，需要对应面板存在后才能完整验证。
...
- 最新运行日志已验证开始按钮触发左转动画，动画结束后调用临时“打开选角面板”回调。
...
制作选角面板，并把开始按钮镜头动画结束后的临时日志替换为选角面板显示逻辑。
