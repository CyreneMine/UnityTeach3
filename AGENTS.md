...
- 本仓库用于系统学习 Unity 核心功能，并保存每一课的练习、验证和复盘记录。
- `UnityTeach3-Theory/` 是已经结束的 96P 理论练习工程，默认只读，仅供参考和复盘。
- `UnityTeach3-Project/` 是后续 38P 3D 综合实践的主工程，代码、场景、Prefab 和项目配置均以该目录为准。
- 仓库根目录保存 `README.md`、`LearningProgress.md`、`CODEX_MEMORY.md` 和 `Notes/` 等跨工程管理资料。
- 学习范围从资源、动画、物理、角色控制和导航，转入 UI、数据、场景、玩家、怪物、关卡和防御塔的完整实践。
- 项目目标不是尽快做完一个成品，而是理解原理、保留练习、建立调试习惯并形成可复查的 GitHub 学习仓库。
- 教程中的实现是学习材料，不自动等同于唯一正确或适合真实项目的方案。

## 2. 当前技术环境

- 两个 Unity 工程的编辑器版本均为 `6000.3.10f1`。
- `UnityTeach3-Theory/` 使用 2D 模板，包含理论课程期间的 2D、动画、物理、角色控制和 AI Navigation 练习。
- `UnityTeach3-Project/` 使用 3D 模板，由原路径 `E:\UnityLearn\UnityTeachDemo3` 整理而来。
- 两个工程当前均使用 Input System `1.18.0` 和 UGUI `2.0.0`。
- 回答和检查综合实践问题时，默认读取 `UnityTeach3-Project/Assets`、`UnityTeach3-Project/Packages` 和 `UnityTeach3-Project/ProjectSettings`。
- Unity 版本或关键包版本改变后，要更新本节并说明原因。
- 不要因为教程版本较旧就盲目照搬已弃用 API；先解释版本差异，再给兼容方案。

## 3. Codex 的职责

- 在回答项目前，先确认问题属于理论参考还是综合实践，再读取对应工程的真实脚本、场景、Prefab、Inspector 配置和学习记录。
...
- 课程目录、复盘表格和说明性文档。
- 用户明确要求维护的截图索引或验证记录。
- `CODEX_MEMORY.md` 中的工作区状态和稳定约定。

### 4.2 默认禁止修改

- `UnityTeach3-Theory/` 下的全部内容默认只读；只有用户明确要求修正理论工程时才可修改。
- `.cs` 脚本。
- `.unity` 场景。
...
- 确认当前工作目录；综合实践默认使用 `UnityTeach3-Project/`，不要把新内容写入 `UnityTeach3-Theory/`。
- 核对课程文件名与记录中的编号和标题。
...
- `CODEX_MEMORY.md` 保存双工程路径、当前主工程和需要跨会话延续的稳定约定。
...
- 提交前排除两个工程内的 `Library/`、`Temp/`、`Logs/`、`obj/`、`UserSettings/` 等生成内容。
- 不混入与当前课程无关的文件。
- 推送后检查本地分支与远程分支是否同步。
- 未经明确允许不改写历史、不强制推送。
...
- 这是学习项目，理解与验证优先于速度。
- 理论工程默认只读；综合实践的新增工作只进入 `UnityTeach3-Project/`。
